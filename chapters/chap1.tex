...
\begin{figure}[H]
    \begin{tikzpicture}
        ...
        \draw[ thick, rotate=180] (0,0) arc (0:90:1.5 and -.5);
...
\begin{figure}[H]
    ...
    \begin{subcaptionbox}
        \centering
        \begin{tikzpicture}[baseline=(current bounding box.center),scale=.6]
            \draw[draw=black] (1.3,1)ellipse (.3 and .6);
            \chemfig{N(-[:-90,1.2]H)(-[:180,1.2]H)(-[:0,1.2]H)};
            \draw[->] (3,0)--(5.5,0)node[above,pos=.5]{不考虑}node[below,pos=.5]{斥力大小};
            \fill (1.4,.6) circle (1pt);
            \fill (1.2,.6) circle (1pt);
        \end{tikzpicture}
    \end{subcaptionbox}
    \begin{subcaptionbox}
        \centering
        \begin{tikzpicture}[baseline=(current bounding box.center),scale=.6]
            \draw[draw=black] (.9,1)ellipse (.3 and .6);
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[baseline=(current bounding box.center),scale=.5]
        ...
        \draw (1.5,1.5)--++(.5,0)node{$\uparrow$}-- ++(.5,0)node[below]{$sp$};
        \draw (11.7,1.5)--++(.5,0)node{$\uparrow$}-- ++(.5,0)node[below]{$sp$};
        \draw (1.5,2.5)--++(.5,0)node{$\uparrow$}-- ++(.5,0)node[below]{$p$};
...
\begin{figure}[H]
    \begin{tikzpicture}[baseline=(current bounding box.center),scale=.4]
        ...
        \draw[-,dashed] (8.3,3.5)--(1.5,1);
        \draw[-,dashed] (8.3,-1.5)--(1.5,1);
        \draw (1.5,1)--++(.5,0)node{$\uparrow$}-- ++(.5,0)node[below]{$1s_H$}node[above]{$\Psi_2$};
    \end{tikzpicture}
    $\left.\begin{matrix}
        C\text{的}sp_C^3 \\
        H\text{的}1s_H
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[baseline=(current bounding box.center),scale=.4]
        ...
        \draw (2.9,1.5)--++(.5,0)node{$\uparrow$}-- ++(.5,0)node[below]{$sp^3$};
        \draw (4.1,1.5)--++(.5,0)node{$\uparrow$}-- ++(.5,0)node[below]{$sp^3$};
        \draw[-] (1.7,.5)--(5.1,.5)node[pos=.5,below]{与3个H的1s成键};
        ...
        \draw[->] (1.5,1.5)--(14,1.5)node[red,pos=.5,above]{简化}node[red,pos=.5,below]{省略组合过程};
